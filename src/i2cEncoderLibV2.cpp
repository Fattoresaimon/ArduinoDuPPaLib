...
/** Add a callback**/
void i2cEncoderLibV2::attachInterrupt(InterruptFunction function, SourceInt event ) {
  Events[event] = function;
}


/** Remove an attached callback **/
void i2cEncoderLibV2::deattachInterrupt(SourceInt event ) {
  Events[event] = NULL;
}
...
void i2cEncoderLibV2::autoconfigInterrupt(void) {
  ...
  for (; i < SOURCE_INT_NUMB; i++) {
    if (Events[i] != NULL) {
      c |= 0x80;
      break;
    }
  }
  writeEncoder(REG_INTCONF, (uint8_t) c);
}


/** Call che attached callaback if it is defined. It's a prive function only **/
void i2cEncoderLibV2::eventcaller(SourceInt event) {
...
bool i2cEncoderLibV2::updateStatus(void) {
  ...
  if (_stat & PUSHR) {
    eventcaller(BUTTON_RELEASE);
  }
  if (_stat & PUSHP) {
    eventcaller(BUTTON_PUSH);
  }
  if (_stat & PUSHD) {
    eventcaller(BUTTON_DOUBLE_PUSH);
  }
  if (_stat & RINC) {
    eventcaller(ENCODER_INCREMENT);
  }
  if (_stat & RDEC) {
    eventcaller(ENCODER_DECREMENT);
  }
  if (_stat & RMAX) {
    eventcaller(ENCODER_MAX);
  }
  if (_stat & RMIN) {
    eventcaller(ENCODER_MIN);
  }
  ...
  if ((_stat & INT2) != 0) {
    ...
    if (_stat2 & GP1_POS) {
      eventcaller(GP1_POSITIVE);
    }
    if (_stat2 & GP1_NEG) {
      eventcaller(GP1_NEGATIVE);
    }
    if (_stat2 & GP2_POS) {
      eventcaller(GP2_POSITIVE);
    }
    if (_stat2 & GP2_NEG) {
      eventcaller(GP2_NEGATIVE);
    }
    if (_stat2 & GP3_POS) {
      eventcaller(GP3_POSITIVE);
    }
    if (_stat2 & GP3_NEG) {
      eventcaller(GP3_NEGATIVE);
    }
    if (_stat2 & FADE_INT) {   
      eventcaller(FADE);
    }
